...
module.exports = {
  ...
  activate() {
    ...
    return require('atom-package-deps').install('pros').then(() => {
      ...
      atom.config.onDidChange('pros.googleAnalytics.enabled', function() {
        if (atom.config.get('pros.googleAnalytics.enabled') &&
          ...
          return GA.startSession();
        } else {
          let sessionControl;
          atom.config.set('pros.googleAnalytics.cid', '');
          return GA.sendData(sessionControl = 'end');
        }
      });
      ...
      atom.commands.add('atom-workspace', {'PROS:New-Project'() { return new (require('./views/new-project')); }});
      atom.commands.add('atom-workspace', { 'PROS:Upgrade-Project'() {
        ...
        if (currentProject[0]) { return new ( require('./views/upgrade-project'))({dir: currentProject[0]});
        } else { return new (require('./views/upgrade-project')); }
      }
    }
      );
      atom.commands.add('atom-workspace', {'PROS:Register-Project'() { return new (require('./views/register-project')); }});
      ...
      atom.commands.add('atom-workspace', {'PROS:Show-Welcome'() { return atom.workspace.open(welcomeUri); }});
      atom.commands.add('atom-workspace', { 'PROS:Toggle-PROS'() {
        return atom.config.set('pros.enable', !atom.config.get('pros.enable'));
      }
    }
      );
      atom.commands.add('atom-workspace', { 'PROS:Open-Conductor'() {
        const currentProject = atom.project.relativizePath(__guard__(atom.workspace.getActiveTextEditor(), x => x.getPath()));
        if (currentProject[0]) {
          return atom.workspace.open(`${conductorUri}/${currentProject[0]}`);
        } else {
          return atom.workspace.open(conductorUri);
        }
      }
    }
      );
      atom.commands.add('atom-workspace', {
        'PROS:Add-Library'() {
          return new (require('./views/add-library'))({path: __guard__(atom.project.getPaths(), x => x[0])});
        }
      });

      this.subscriptions.add(atom.workspace.addOpener(function(uri) {
        if (uri === welcomeUri) { return WelcomeView != null ? WelcomeView : (WelcomeView = new (require('./views/welcome'))); }
      })
      );
      ...
      cli.execute({
        ...
        cb(c, o, e) { if (o) { return console.log(o); } }
      });
      ...
      return grammarSubscription = atom.grammars.onDidAddGrammar(function(grammar) {
  ...
  consumeToolbar(toolBarRegister) {
    ...
    return toolBar.onDidDestroy(() => toolBar = null);
  },

  autocompleteProvider() { return autocomplete.provide(); },

  consumeStatusBar(statusbar) {
    return Status.attach(statusbar);
  },
