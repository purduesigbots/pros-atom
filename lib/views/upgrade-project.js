...
module.exports =
  (UpgradeProjectModal = class UpgradeProjectModal extends View {
      ...
      return this.div({class: 'pros-modal pros-upgrade-project'}, () => {
        ...
        return this.div({class: 'actions'}, () => {
          this.div({class: 'btn-group'}, () => {
            ...
            return this.button({outlet: 'upgradeButton', class: 'btn btn-primary icon icon-rocket'}, 'Upgrade');
          });
    ...
    initialize(param) {
      ...
      this.upgradeButton.prop('disabled', true);
      this.projectPathEditor.getModel().onDidChange(() => {
        return this.upgradeButton.prop('disabled', !!!this.projectPathEditor.getText());
      });

      this.toggleListButton.click(() => $('#projectPathPicker ol').toggleClass('enabled'));
      this.projectPathPicker.find('li').on('click', e => this.projectPathEditor.setText($(e.target).text()));
