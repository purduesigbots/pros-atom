...
module.exports = {
  ...
  getValidGrammar(editor) {
    ...
    if (grammar_name === 'C') { return 'C'; 
    } else if (grammar_name.indexOf('C++' >= 0)) { return 'C++'; 
  ...
  lint(editor, lintable_file, real_file) {
    ...
    return commandExists(command, (err, commandDoesExist) => {
      if (err) {
        ...
        const flags = (() => {
          if (grammar_type = 'C++') { return settings.lint.default_Cpp_flags; 
          } else if (grammar_type = 'C') { return settings.lint.default_C_flags; }
        })();
